...
\begin{equation}
    ...
    \end{cases}
\end{equation} \newline
$I$ represents the number of fixed subproblems to be iterated. If $N_1 \% \mu$ is equal to zero, which means that 


% Hence, for this BESS prediction control problem, the time complexity is:\newline
% \begin{equation}
%     f(n)=O(k^{N_1})
% \end{equation}\newline
% $k$ is a constant depending on the number of branches has been created by the branch and bound methods in the MILP model and $N_1$ represents the number of decision variables. It is significant that the actual number of decision variables is $2*N_1$ since at each time interval the energy has been divided into two parts-one for energy under peak limit, and another for excessive. This constant $2$ is ignored because it doesn't influent the time complexity. Relatively, the time complexity of the rolling horizon approach based on MILP is:\newline
% \begin{equation}
%     f(n)=O(k^{\mu}N_1)
% \end{equation}\newline
% $\mu$ is the size of the fixed subproblem which is a constant far less than the number of the time intervals or original problems' decision variables $N_1$. Since the exponential part plays a dominant role in time complexity and $\mu$ is less than $N_1$ which means $2^{\mu}$ is far less than $2^{N_1}$, the time complexity has been improved by applying rolling horizon approach to the MILP model. 
...
\begin{cases}
    ...
    \text{Fully Discharge}, & \text{else if } L_{i-23}<PL \text{ and } L_{i-22}<PL \\ \text{ and } L_i<0.8PL  \\
